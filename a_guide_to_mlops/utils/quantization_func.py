...
def evaluate_base_model(bentomodel_tag, ds_test, labels):

    # Load the model using BentoML
    model_name = os.path.basename(bentomodel_tag).split(".")[0]  # Remove extension
    ...
    for images, label_idxs in ds_test:
        for image, true_label in zip(images, label_idxs):
            ...
            # Preprocess image for BentoML model (assuming float32 input)
